...
const N8N_WEBHOOK_URL = 'https://n8n-bs-comunicaciones-n8n.jz98vr.easypanel.host/webhook';
...
let state = {
    ...
    availableTypes: [] // Para guardar los tipos disponibles
};
...
function renderRooms() {
    ...
    // Filtrar habitaciones disponibles usando lo que devolvió n8n
    // Se compara ignorando mayúsculas/minúsculas y tildes
    const isAvailable = (room) => {
        if (!state.availableTypes || state.availableTypes.length === 0) return true; // fallback
        const normalize = (s) => s.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
        const roomNameNorm = normalize(room.name);
        const roomIdNorm = normalize(room.id);
        
        return state.availableTypes.some(av => {
            const avNorm = normalize(av);
            return avNorm.includes(roomNameNorm) || roomNameNorm.includes(avNorm) || 
                   avNorm.includes(roomIdNorm) || roomIdNorm.includes(avNorm);
        });
    };

    const availableRooms = ROOM_TYPES.filter(isAvailable);

    if (availableRooms.length === 0) {
        list.innerHTML = `<div style="text-align:center; padding: 2rem; color: #666;">
            <i class="fas fa-frown" style="font-size: 2rem; margin-bottom: 1rem; color: #ccc;"></i>
            <p>Lo sentimos, no hay habitaciones disponibles para estas fechas.</p>
            <p style="font-size: 0.8rem; margin-top: 0.5rem;">Intenta con otras fechas o contáctanos por WhatsApp.</p>
        </div>`;
    ...
    if (state.preselectedRoomId) {
        const room = availableRooms.find(r => r.id === state.preselectedRoomId);
        if (room) {
            ...
            // Estaba preseleccionada pero no está disponible
            state.preselectedRoomId = null;
        }
    }
}
...
function initBookingForm() {
    ...
    if (heroForm) {
        heroForm.addEventListener('submit', async (e) => {
            ...
            // Validar disponibilidad con n8n
            const submitBtn = heroForm.querySelector('button[type="submit"]');
            const originalBtnText = submitBtn.textContent;
            submitBtn.textContent = 'Buscando disponibilidad...';
            submitBtn.disabled = true;

            try {
                const response = await fetch(`${N8N_WEBHOOK_URL}/check-availability?check_in=${checkIn}&check_out=${checkOut}`);
                const data = await response.json();
                
                // data debería ser un array de objetos desde la base de datos
                if (Array.isArray(data)) {
                    state.availableTypes = [...new Set(data.map(r => r.room_type || r.type || ''))];
                } else {
                    state.availableTypes = []; // si no es array, asumimos nada disponible o error
                }
            } catch (err) {
                console.error('Error validando disponibilidad en n8n:', err);
                // Fallback: si falla n8n, mostramos todas para no bloquear ventas (o podemos bloquearlas)
                state.availableTypes = ROOM_TYPES.map(r => r.name); 
            } finally {
    ...
    if (guestForm) {
        guestForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                // Insertar mediante n8n webhook (seguro y no expone Supabase key al público)
                const response = await fetch(`${N8N_WEBHOOK_URL}/create-booking`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(payload)
                });
                
                if (!response.ok) throw new Error('Error en el servidor al enviar la reserva.');
                const responseData = await response.json();
                
                // n8n inserta y devuelve la fila de Supabase (si el nodo está en lastNode mode)
                let refCode = '—';
                if (Array.isArray(responseData) && responseData[0] && responseData[0].id) {
                    refCode = String(responseData[0].id).slice(0, 8).toUpperCase();
                } else if (responseData.id) {
                    refCode = String(responseData.id).slice(0, 8).toUpperCase();
                } else {
                    refCode = Math.random().toString(36).substring(2, 10).toUpperCase(); // fallback
                }
